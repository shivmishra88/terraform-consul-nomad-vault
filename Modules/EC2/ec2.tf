...
user_data = <<-EOF
              ...
              sudo apt-get install net-tools && sudo apt-get install default-jdk -y
              ...
              sudo mkdir --parents /etc/nomad.d && sudo mkdir --parents /var/nomad
              sudo chown --recursive nomad:nomad /etc/nomad.d && sudo chown --recursive nomad:nomad /var/nomad
              wget https://releases.hashicorp.com/nomad/1.5.5/nomad_1.5.5_linux_amd64.zip
              unzip nomad_1.5.5_linux_amd64.zip && sudo mv nomad /usr/local/bin/
              sudo chmod 755 /usr/local/bin/nomad && sudo chown nomad:nomad /usr/local/bin/nomad 
              else
              ...
              sudo service consul restart && sudo service nomad restart
              fi
              ...
              if [ ${count.index} -eq 0 ]; then
                  ...
                  sudo useradd --system --home /etc/vault.d --shell /bin/false vault
                  sudo mkdir --parents /etc/vault.d && sudo chown --recursive vault:vault /etc/vault.d
                  ...
                  sudo touch /etc/systemd/system/netmaster.service && sudo touch /etc/systemd/system/netplugin.service
              ...
              if [ ${count.index} -eq 0 ] || [ ${count.index} -eq 1 ] || [ ${count.index} -eq 2 ]; then
                  ...
                  sudo service netmaster restart && sudo service netplugin restart
              elif [ ${count.index} -eq 3 ] || [ ${count.index} -eq 4 ] || [ ${count.index} -eq 5 ] || [ ${count.index} -eq 6 ]; then
                  echo "${file("${path.module}/../../netplugin.service.tpl")}" | sudo tee /etc/systemd/system/netplugin.service
                  sudo service openvswitch-switch restart
                  sudo /usr/share/openvswitch/scripts/ovs-ctl restart && sudo service netplugin restart 
              else
                  echo "Else nothing"
              fi
                  echo "Installation has been done"
               if [ ${count.index} -ge 7 ] && [ ${count.index} -le 9 ]; then
                  ...
                  sudo mv /opt/zookeeper-3.4.9 /opt/zookeeper && sudo mkdir -p /var/lib/zookeeper/data
               ...
               if [[ ${count.index} -ge 7 && ${count.index} -le 11 ]]; then
                  ...
                  sudo apt-get update && sudo apt-get install dse-full -y
                  sudo systemctl daemon-reload && sudo service dse start
